...
2. **Custom relay URL** — when the panel reports the relay isn't reachable, paste the base URL of a self-hosted copy into the ⚡ panel's field (it persists in localStorage and is tried first). Self-hosting is one command with zero dependencies: copy `api/freedns.py` to any always-on box and run `python3 api/freedns.py` — it serves JSON with open CORS on `0.0.0.0:8787` (pass a port as the first argument, or set `$PORT`). No spare box? Run it **free on Koyeb**: sign up (no credit card) → *Create Service* → *Docker* → point it at your repo containing `api/freedns.py` with the Dockerfile below (or run the file directly with the Python run command `python api/freedns.py`) → Koyeb gives you a free `https://<app>.koyeb.app` URL with HTTPS and its own health checks (the relay answers `/` with a liveness 200 and honors `$PORT`). Paste `https://<app>.koyeb.app/api/freedns` into the ⚡ panel — note Koyeb's free instance sleeps after an hour of no traffic (a few seconds' wake-up on the next click), and the in-memory FreeDNS sessions reset when it does, so re-login after a sleep.
