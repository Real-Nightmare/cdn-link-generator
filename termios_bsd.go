...
// getTermios fetches current terminal attributes (BSD/macOS ioctls).
...
func disableEcho(fd int, t *syscall.Termios) error {
	...
	_, _, errno := syscall.Syscall(syscall.SYS_IOCTL, uintptr(fd), uintptr(syscall.TIOCSETA), uintptr(unsafe.Pointer(&noEcho)))
	if errno != 0 {
		return errno
	}
	return nil
